"""
eval_real_robot_nova5.py
========================
越疆 DOBOT Nova 5 真机部署推理脚本。

参考 eval_real_robot_flexiv.py 实现，支持：
  - LDP（Latent Diffusion Policy）推理部署（快慢双系统）
  - AT（Asymmetric Tokenizer）仅作为 LDP 的子模块使用

使用方法：
----------
## 1. 启动 DOBOT Nova 5 机器人 HTTP 服务器（在另一个终端）
...
- task           : 使用 nova5_rdp_image_tactile_emb_ldp_24fps 任务配置
...
------------------
                  ...
                  │  → HTTP → Nova5Server → ServoP → Nova 5       │
                  └──────────────────────────────────────────────┘
"""
