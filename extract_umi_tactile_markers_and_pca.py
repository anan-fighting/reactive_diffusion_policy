...
触觉标记点提取 + PCA 降维脚本
============================
针对 ViTai GF225 传感器，从 tactile_left.mp4 / tactile_right.mp4 中提取
2D 标记点坐标，并对位移场做 PCA 降维，生成 RDP 训练所需的三个数组：

    initial_marker     [T, 81, 2]   第 0 帧标记点坐标（归一化到 [0,1]）
    marker_offset      [T, 81, 2]   每帧相对第 0 帧的位移（归一化）
    marker_offset_emb  [T, 15]      PCA 降维后的触觉嵌入

工作流程：
    Step 1  遍历所有 episode，用 pyvitaisdk 提取 marker_origin / marker_offset，
            保存为 initial_marker.npy 和 marker_offset.npy（归一化）
    Step 2  汇总所有 episode 的 marker_offset，训练 PCA（n_components=15），
            保存变换矩阵 W 和均值 mean 到 data/PCA_Transform_Custom/
    Step 3  对每个 episode 应用 PCA，保存 marker_offset_emb.npy

用法：
    # 完整流程（推荐首次运行）
    python extract_tactile_markers_and_pca.py

    # 只做 Step1（提取标记点，跳过 PCA）
    python extract_tactile_markers_and_pca.py --step 1

    # 只做 Step2+3（已有 .npy，重新训练 PCA 并生成嵌入）
    python extract_tactile_markers_and_pca.py --step 23

    # 只处理特定 side
    python extract_tactile_markers_and_pca.py --sides left

    # 单 episode 快速调试
    python extract_tactile_markers_and_pca.py --episode episode_0 --step 1
...
CONFIG = {
    # 数据集根目录
    "episodes_root": "/home/zzw/project/umi-act/ur5e_datasets/pick_up_and_place_blocks_sync_0416",

    # episode 目录前缀
    "episode_prefix": "episode_",

    # 每个 episode 内触觉数据子目录（含 tactile_*.mp4）
    "hand_subdir": "left_hand",

    # 处理哪些 side（对应视频名 tactile_{side}.mp4）
    "sides": ["left", "right"],

    # 图像宽高（用于归一化），会自动从视频中读取，此处仅作默认值
    "image_wh": (240, 240),
...
def step1_extract_all(episodes_root: str, episodes: list, hand_subdir: str,
                       sides: list, bg_frame_index: int, sensor_type_str: str,
                       log_interval: int):
    """遍历所有 episode，提取并保存 initial_marker.npy 和 marker_offset.npy。"""
    print("\n" + "="*60)
    print("Step 1：提取标记点坐标")
    print("="*60)

    for ep_name in episodes:
        hand_dir = Path(episodes_root) / ep_name / hand_subdir
        for side in sides:
            video_path = hand_dir / f"tactile_{side}.mp4"
            if not video_path.exists():
                print(f"[SKIP] {video_path} 不存在")
                continue

            out_initial = hand_dir / f"initial_marker_{side}.npy"
            out_offset = hand_dir / f"marker_offset_{side}.npy"
            ...
            try:
                ...
                print(f"  已保存: initial_marker_{side}.npy  shape={initial_marker.shape}")
                print(f"  已保存: marker_offset_{side}.npy   shape={marker_offset.shape}")
            except Exception as e:
...
def step2_train_pca(episodes_root: str, episodes: list, hand_subdir: str,
                     sides: list, n_components: int, pca_save_dir: str):
    """
    ...
    for side in sides:
        ...
        for ep_name in episodes:
            offset_path = Path(episodes_root) / ep_name / hand_subdir / f"marker_offset_{side}.npy"
...
def step3_apply_pca(episodes_root: str, episodes: list, hand_subdir: str,
                     sides: list, n_components: int, pca_save_dir: str):
    """
    对每个 episode 的 marker_offset 应用已训练的 PCA，保存 marker_offset_emb.npy。
    """
    print("\n" + "="*60)
    ...
    for side in sides:
        ...
        for ep_name in episodes:
            hand_dir = Path(episodes_root) / ep_name / hand_subdir
            offset_path = hand_dir / f"marker_offset_{side}.npy"
            out_path = hand_dir / f"marker_offset_emb_{side}.npy"

            if not offset_path.exists():
                continue

            if out_path.exists():
                pass  # 强制重新生成（PCA 矩阵可能已更新）
        ...
        sample_ep = next((e for e in episodes
                          if (Path(episodes_root)/e/hand_subdir/f"marker_offset_emb_{side}.npy").exists()), None)
        if sample_ep:
            sample = np.load(str(Path(episodes_root)/sample_ep/hand_subdir/f"marker_offset_emb_{side}.npy"))
            print(f"  示例 ({sample_ep}): marker_offset_emb_{side}.npy  shape={sample.shape}  dtype={sample.dtype}")

        total = sum(1 for e in episodes
                    if (Path(episodes_root)/e/hand_subdir/f"marker_offset_emb_{side}.npy").exists())
        print(f"  共生成 {total} 个 episode 的 marker_offset_emb_{side}.npy")


# ─────────────────────────── 验证工具 ─────────────────────────────────────────

def verify(episodes_root: str, episodes: list, hand_subdir: str, sides: list):
    """快速验证输出文件是否完整。"""
    print("\n" + "="*60)
    print("验证输出")
    print("="*60)
    for side in sides:
        for fname in [f"initial_marker_{side}.npy",
            ...
            found = sum(
                1 for e in episodes
                if (Path(episodes_root)/e/hand_subdir/fname).exists()
            )
            print(f"  {fname:40s}  {found}/{len(episodes)} episodes")


# ─────────────────────────── 入口 ─────────────────────────────────────────────

def main():
    parser = argparse.ArgumentParser(description="触觉标记点提取 + PCA 降维")
    parser.add_argument("--step", type=str, default="123",
                        help="执行步骤：'1'=提取, '2'=训练PCA, '3'=应用PCA, '123'=全部")
