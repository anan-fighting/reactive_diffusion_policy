#!/bin/bash
# ================================================================
# 用 UMI 自定义数据集训练 RDP（AT + LDP）
# 使用前请先运行数据转换脚本：
#   python extract_tactile_markers_and_pca.py # 生成触觉相关的数据
#   python scripts/convert_umi_to_zarr.py # 将原始数据转换为 zarr 格式
# ================================================================
...
CUDA_VISIBLE_DEVICES=${GPU_ID} python train.py \
    ...
    task=umi_rdp_image_tactile_emb_at_24fps \
    task.dataset_path=${DATASET_PATH} \
    task.name=umi_rdp_image_tactile_emb_at_24fps_${TIMESTAMP} \
    at=at_umi_rdp \
